...
from decompose.safety import MANAGED_WORKSPACE_DIRECTORIES, workspace_boundary_diagnostics


def resolve_workspace(explicit: Path | None = None) -> Path:
    if explicit is not None:
        return explicit.expanduser().resolve()
    configured = os.environ.get("TASKSPEC_WORKSPACE_ROOT")
    if configured:
        return Path(configured).expanduser().resolve()
    current = Path.cwd().resolve()
    for candidate in (current, *current.parents):
        if (candidate / PLAN_DIR / "intent.md").is_file():
            return candidate
    try:
...
def init_workspace(root: Path, *, force: bool = False, dry_run: bool = False) -> Result:
    ...
    starter_documents = [
        root / PLAN_DIR / "intent.md",
        root / PLAN_DIR / "system-map.md",
    ]
    derived_starters = [
        root / PLAN_DIR / "evidence.jsonl",
        root / PLAN_DIR / "seam-map.yaml",
        root / PLAN_DIR / "steel-thread.md",
    ]
    ...
    if not force and collisions:
        return Result(
            ...
            next_steps=["taskspec decompose status <initiative>"],
        )
    if not dry_run:
...
"""
    ...
    return Result(
        ...
        next_steps=["Author evidence, then run: taskspec decompose prepare <initiative> --recipe <recipe.yaml>"],
        data={"dry_run": dry_run},
    )


def _stage_state_unlocked(root: Path) -> dict[str, Any]:
    from decompose.engine import derive_task_bundle, verify_plan, verify_seam_map
    from decompose.taskspec_adapter import (
    ...
    seam_map = root / PLAN_DIR / "seam-map.yaml"
    plan = root / PLAN_DIR / "delivery-plan.yaml"
    review = root / PLAN_DIR / "reviews" / "delivery-plan-review.json"
...
def next_steps_for_state(state: dict[str, Any]) -> list[str]:
    ...
    if state["issues"]:
        ...
        return ["taskspec decompose init <initiative> --intent-file <file>"]
    elif not state["seam_map"]:
        return ["taskspec decompose prepare <initiative> --recipe <recipe.yaml>"]
    elif not state["delivery_plan"]:
        return ["taskspec decompose prepare <initiative> --recipe <file> --replace"]
    elif not state["reviewed"]:
        return ["taskspec decompose review <initiative> --accept --reviewer <name> --reason <reason>"]
    elif not state["task_graph"]:
        return ["taskspec decompose compile <initiative>"]
    return [
        "Use the initiative task-plan.json and task-plan-lineage.json with taskspec plan and batch."
    ]
