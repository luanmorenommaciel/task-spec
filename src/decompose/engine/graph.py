...
from decompose.engine.support import (
...
def _graph_projection(
    ...
    id_set = set(ids)
    for task in tasks:
        unknown_capabilities = set(task.get("proves_capabilities", [])) - {r["leg_id"] for r in records}
        if unknown_capabilities:
            diagnostics.append(_diag("unknown_capability_proof", "Integration proof names unknown capability legs.", ids=sorted(unknown_capabilities)))
    ...
    for left, right in itertools.combinations(tasks, 2):
        dual_creates = {a for a in left["creates_paths"] for b in right["creates_paths"] if _paths_overlap(a, b)}
        if dual_creates:
            diagnostics.append(_diag("dual_create_collision", "Two leaves claim creation of the same write surface; assign one creator.", paths=sorted(dual_creates)))
        left_paths = {
            value
        ...
        overlap += ["resource:" + value for value in sorted(set(left.get("shared_resources", [])) & set(right.get("shared_resources", [])))]
        if not overlap:
            continue
