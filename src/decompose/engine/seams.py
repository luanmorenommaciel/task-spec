...
from decompose.engine.recipe import (
    _load_recipe,
    _semantic_recipe_checks,
    _verify_local_recipe_sources,
    _write_path_state_diagnostics,
)
from decompose.engine.support import _canonical_project_path, _diag, _event
...
def map_recipe(root: Path, source: Path, *, dry_run: bool = False) -> Result:
    ...
    with workspace_lock(root, dry_run=dry_run):
        ...
        intent_path = root / PLAN_DIR / "intent.md"
        ...
        system_map_path = root / PLAN_DIR / "system-map.md"
        writer.text(
            system_map_path,
            system_map_text,
        )
        ...
        evidence_path = root / PLAN_DIR / "evidence.jsonl"
        ...
        for decision in recipe.get("decisions", []):
            decision_path = root / PLAN_DIR / "decisions" / f"{decision['id']}.md"
        ...
        for seam in recipe["seams"]:
            path = root / PLAN_DIR / "seams" / f"{seam['id']}.md"
...
def _verify_seam_sources(root: Path, seam_map: dict[str, Any]) -> list[Diagnostic]:
    ...
    for record in seam_map.get("decisions", []):
        ...
        if not isinstance(raw_path, str):
            ...
            continue
        decision_path = _owned_artifact_path(root, raw_path, (PLAN_DIR + '/decisions'))
    ...
    actual_decisions = set((root / PLAN_DIR / "decisions").glob("*.md"))
    ...
    for record in seam_map["seams"]:
        ...
        if not isinstance(raw_path, str):
            ...
            continue
        seam_path = _owned_artifact_path(root, raw_path, (PLAN_DIR + '/seams'))
    ...
    actual_seams = set((root / PLAN_DIR / "seams").glob("*.md"))
...
def verify_seam_map(root: Path) -> tuple[dict[str, Any] | None, list[Diagnostic]]:
    path = root / PLAN_DIR / "seam-map.yaml"
    if not path.is_file():
