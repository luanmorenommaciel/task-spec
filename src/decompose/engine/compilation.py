...
from decompose.engine.graph import _graph_projection, _task_records
from decompose.engine.planning import verify_plan
from decompose.engine.support import _diag
...
def derive_task_bundle(
    ...
    """Deterministically rebuild TaskSpec decomposition-owned topology before materialization."""

    plan_sha = sha256_file(root / PLAN_DIR / "delivery-plan.yaml")
    ...
    *,
    ...
    if plan is None:
        ...
        return Result(
            ...
            next_steps=[
                "taskspec decompose prepare <initiative> --recipe <file> --replace",
                "taskspec decompose review <initiative> --accept --reviewer <name> --reason <reason>",
            ],
        )
