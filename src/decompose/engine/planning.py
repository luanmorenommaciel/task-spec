...
from decompose.auth import sign, verify, reviewed_inputs
...
from decompose.constants import (
    EXIT_CONFLICT,
    EXIT_INVALID,
    EXIT_NEEDS_INPUT,
    EXIT_OK,
...
from decompose.contracts import validate_contract
from decompose.engine.seams import _owned_artifact_path, verify_seam_map
from decompose.engine.support import _diag, _event
...
from decompose.render import (
    render_leg,
    render_steel_thread,
    render_swimlane,
)
from decompose.result import Diagnostic, Result
...
def build_plan(root: Path, *, dry_run: bool = False) -> Result:
    ...
    seam_map_path = root / PLAN_DIR / "seam-map.yaml"
    ...
    if seam_map is None:
        ...
        return Result(
            ...
            next_steps=['taskspec decompose prepare <initiative> --recipe "/path/to/recipe.yaml"'],
        )
    system_path = root / PLAN_DIR / "system-map.md"
    try:
    ...
    with workspace_lock(root, dry_run=dry_run):
        ...
        for seam_record in seam_map["seams"]:
            ...
            lane = seam["swimlane"]
            lane_path = root / PLAN_DIR / "swimlanes" / f"{lane['id']}.md"
            ...
            for leg in lane["legs"]:
                leg_path = root / PLAN_DIR / "legs" / f"{leg['id']}.md"
        ...
        steel_thread_path = root / PLAN_DIR / "steel-thread.md"
        ...
        existing_path = root / PLAN_DIR / "delivery-plan.yaml"
        receipt_path = root / PLAN_DIR / "reviews" / "delivery-plan-review.json"
        if existing_path.is_file() and receipt_path.is_file():
            ...
            if (
                ...
                return Result(
                    ...
                    next_steps=["taskspec decompose compile <initiative>"],
    ...
    next_steps = (
        ["Resolve every OPEN objection, then rerun: taskspec decompose prepare <initiative> --recipe <file> --replace"]
        if token == PLAN_OPEN_OBJECTIONS
        else ["taskspec decompose review <initiative> --accept --reviewer <name> --reason <reason>"]
    )
    return Result(
        "plan",
        token,
...
def accept_plan(
    ...
    *,
    ...
    plan_path = root / PLAN_DIR / "delivery-plan.yaml"
    ...
    if plan is None:
        return Result(
            ...
            next_steps=["taskspec decompose prepare <initiative> --recipe <file> --replace"],
        )
    ...
    if plan.get("seam_map_sha256") != sha256_file(root / PLAN_DIR / "seam-map.yaml"):
        return Result(
            "review",
            PLAN_NEEDS_REVIEW,
            EXIT_CONFLICT,
    ...
    authority_record = {
        ...
        "topology_sha256": sha256_file(root / PLAN_DIR / "proposed-graph.json"),
        "recipe_sha256": sha256_file(root / PLAN_DIR / "source-recipe.yaml"),
        "artifacts_sha256": reviewed_inputs(root / PLAN_DIR),
        "fixture": fixture,
    }
    ...
    try:
        receipt["signature"] = sign(root, receipt)
    except (OSError, ValueError) as error:
        return Result("review", PLAN_NEEDS_REVIEW, EXIT_INVALID, root, diagnostics=[_diag("review_key_unavailable", str(error))])
...
def verify_plan(
    ...
    for kind, prefix in (("swimlanes", (PLAN_DIR + '/swimlanes')), ("legs", (PLAN_DIR + '/legs'))):
        items = plan.get(kind, [])
    ...
    for kind in ("swimlanes", "legs"):
        actual_paths = set((root / PLAN_DIR / kind).glob("*.md"))
    ...
    if isinstance(steel_path_value, str):
        steel_path = _owned_artifact_path(root, steel_path_value, PLAN_DIR)
        if (
            steel_path is None
    ...
    if require_review:
        if not receipt_path.is_file():
            ...
            try:
                ...
                if isinstance(receipt, dict) and (receipt.get("topology_sha256") != sha256_file(root / PLAN_DIR / "proposed-graph.json") or receipt.get("recipe_sha256") != sha256_file(root / PLAN_DIR / "source-recipe.yaml")):
                    diagnostics.append(_diag("review_snapshot_changed", "Reviewed recipe or topology changed.", receipt_path))
                if isinstance(receipt, dict) and not verify(root, receipt):
                    diagnostics.append(_diag("review_signature_invalid", "Native review authentication failed.", receipt_path))
                if isinstance(receipt, dict) and receipt.get("artifacts_sha256") != reviewed_inputs(root / PLAN_DIR):
                    diagnostics.append(_diag("review_inputs_changed", "Reviewed input content or inventory changed; prepare and review again.", receipt_path))
                if isinstance(receipt, dict):
                    authority_record = {
                        key: value for key, value in receipt.items() if key not in {"plan_sha256", "signature"}
                    }
