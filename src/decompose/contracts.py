"""Load and validate versioned TaskSpec decomposition JSON Schemas."""
...
def source_repository_root() -> Path | None:
    ...
    source_file = candidate / "src" / PLAN_DIR / "contracts.py"
    if (
...
def schema_path(name: str) -> Path:
    return Path(__file__).resolve().parents[2] / "spec" / "schemas" / ("decompose-" + name + ".schema.json")
