...
TaskSpec decomposition owns reviewed decomposition and lineage. It deliberately does not
...
from decompose.constants import VERSION
...
TASK_PLAN_LINEAGE_CONTRACT = "TaskPlanLineage/v1"
TASK_PLAN_PATH = Path((PLAN_DIR + '/task-plan.json'))
TASK_PLAN_LINEAGE_PATH = Path((PLAN_DIR + '/task-plan-lineage.json'))
...
def build_task_plan(
    ...
    """Project reviewed TaskSpec decomposition units into Task-Spec's public TaskPlan contract."""

    intent, _ = load_frontmatter(root / PLAN_DIR / "intent.md")
    plan_path = root / PLAN_DIR / "delivery-plan.yaml"
    review_path = root / PLAN_DIR / "reviews" / "delivery-plan-review.json"
        ...
        units.append(
            {
                ...
                **{k: task[k] for k in ("execution_recipe", "shared_resources", "proves_capabilities", "sdlc_stages") if k in task},
...
def verify_task_plan_bundle(
    ...
    """Verify only TaskSpec decomposition-owned projections; Task-Spec validation stays external."""
    ...
    if actual_plan != expected_plan:
        diagnostics.append(
            Diagnostic(
                ...
                "TaskPlan differs from the current reviewed TaskSpec decomposition projection.",
                str(plan_path),
            )
        )
