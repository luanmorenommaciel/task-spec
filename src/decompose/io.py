    ...
    def commit(self) -> None:
        ...
        try:
            ...
            for path, temp_path in staged.items():
                ...
                if path.exists():
                    backup = path.parent / f".{path.name}.taskspec-backup-{os.getpid()}"
                    if backup.exists():
...
def private_state_path(root: Path, *parts: str) -> Path:
    ...
    try:
        output = subprocess.run(
            ["git", "rev-parse", "--git-path", PLAN_DIR],
            cwd=resolved_root,
            check=True,
            capture_output=True,
            text=True,
        ...
        if configured:
            ...
            state_home = Path(os.environ["XDG_STATE_HOME"]).expanduser().resolve() / PLAN_DIR
        elif sys.platform == "darwin":
            state_home = Path.home() / "Library" / "Application Support" / PLAN_DIR
        else:
            state_home = Path.home() / ".local" / "state" / PLAN_DIR
...
def workspace_lock_path(root: Path) -> Path:
    ...
    resolved_root = root.resolve()
    digest = hashlib.sha256(str(resolved_root).encode("utf-8")).hexdigest()[:20]
    configured = os.environ.get("TASKSPEC_PLAN_LOCK_HOME")
    if configured:
        lock_home = Path(os.path.abspath(os.path.expanduser(configured)))
    else:
        user_id = str(os.getuid()) if hasattr(os, "getuid") else "user"
        lock_home = Path(tempfile.gettempdir()).resolve() / f"taskspec-plan-locks-{user_id}"
...
def workspace_lock(root: Path, *, dry_run: bool = False) -> Iterator[None]:
    ...
    if dry_run or identity in _HELD_LOCKS:
        yield
        return
    import fcntl
