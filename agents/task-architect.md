...
### Primary: engine-repo KB

```text
<task-spec repo>/
├── spec/task-spec-v3.md         ← THE format spec
├── docs/concepts/
...
See [../docs/concepts/profiles.md](../docs/concepts/profiles.md).
...
See [../docs/concepts/six-zones.md](../docs/concepts/six-zones.md) for the Behavior zone anatomy and [../docs/concepts/profiles.md](../docs/concepts/profiles.md) for the traceability rule.
...
2. **Contract (Success Criteria)**: write 3-5 runnable bash evals
   ...
   - **NEVER use the inverted-grep-c footgun `count=$(grep -c X file || echo 0); [ "$count" -eq 0 ]`** — it produces the string `"0\n0"` on zero matches and silently inverts the success semantic. Use `! grep -q PATTERN file` instead. See [../docs/patterns/runnable-bash-evals.md](../docs/patterns/runnable-bash-evals.md) "Common foot-guns".
...
1. Print to stdout the exact command the human author should run next:
   ```
   Next: taskspec gate --stamp tasks/T-<your-spec>.md
   ```
...
See [../docs/concepts/signed-off.md](../docs/concepts/signed-off.md) for the full validate-vs-gate contract.
...
See [../docs/concepts/conformance-levels.md](../docs/concepts/conformance-levels.md).
...
See [../docs/concepts/agent-contract.md](../docs/concepts/agent-contract.md)
and [../docs/concepts/conformance-levels.md](../docs/concepts/conformance-levels.md).
