...
kind: seam
claim: derived
id: SEAM-THREAD-PROOF
name: Steel thread integration proof
description: Owns the end-to-end proof that the four source steps hold in the stated order once the other
  seams are composed.
evidence:
- EV-BLUEPRINT
responsibility: Prove the composed steel thread end to end without owning any production behavior.
consumes:
...
independent_proof: The integration test fails if any upstream leg regresses, and it adds no production
  write surface of its own.
rejected_alternatives:
- alternative: Rely on the four leg proofs and skip an integration test.
  reason: Passing leg proofs leave the composed capability unproven; wiring defects between resolution,
    counting, reason, and telemetry would go undetected.
- alternative: Attach the end-to-end assertion to the telemetry leaf.
  reason: That leaf would then own two outcomes and could pass by weakening the composed assertion rather
    than fixing composition.
swimlane:
  id: LANE-THREAD-PROOF
  name: Integration proof lane
  owner: UNASSIGNED placeholder (see DEC-OWNER-UNASSIGNED)
  legs:
  - id: LEG-THREAD-PROOF
    observable_state: 'One end-to-end test walks the source order: invalid policy rejected, exactly one
      effective policy resolved, request 101 denied after 100 allowed, stable reason returned with matching
      telemetry.'
    ...
    - id: T-20260915-steel-thread-proof
      ...
      done_condition: One integration test rejects an invalid policy, resolves exactly one effective policy
        for an organization, allows 100 requests and denies request 101 in the same window, and asserts
        the stable reason with a matching telemetry record; it passes and no production module is modified
        by this leaf.
      effort: S
      profile: standard
      ...
      - id: B-1
        ...
        when: an invalid policy is validated and then 101 requests are evaluated in one window against
          a resolved limit of 100
        then: the invalid policy is rejected, exactly one effective policy is resolved, requests 1 to
          100 are allowed, and request 101 is denied
      - id: B-2
      ...
      - action: Editing production modules to make the integration test pass.
        reason: This leaf proves composition; repairing behavior here hides which sealed leaf actually
          regressed.
      ...
      - test
---
# Steel thread integration proof

Owns the end-to-end proof that the four source steps hold in the stated order once the other seams are composed.

## Responsibility

Prove the composed steel thread end to end without owning any production behavior.

## Independent proof

The integration test fails if any upstream leg regresses, and it adds no production write surface of its own.

## Rejected alternatives

- **Rely on the four leg proofs and skip an integration test.** — Passing leg proofs leave the composed capability unproven; wiring defects between resolution, counting, reason, and telemetry would go undetected.
- **Attach the end-to-end assertion to the telemetry leaf.** — That leaf would then own two outcomes and could pass by weakening the composed assertion rather than fixing composition.
