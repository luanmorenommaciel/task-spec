...
def recipe(issue,work,evaluator,harness):
    ...
    for leaf in leaves(issue):
        ...
        task['evals'] += [{'id':'eval_2','description':'Declared write surface only','bash':shlex.join([sys.executable,str(evaluator),'write-boundary','--root','.']),'verifies':['B-2']},{'id':'eval_3','description':'Registered issue evidence is unchanged','bash':shlex.join([sys.executable,str(evaluator),'source-integrity','--root','.']),'verifies':['B-2']}]
