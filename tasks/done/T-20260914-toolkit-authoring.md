...
agent: codex
parent: (none)
depends_on: [T-20260914-toolkit-decomposition]
supersedes: (none)
touches_paths: [src/author, src/security, src/dispatch/handoff.py]
creates_paths: [src/recipe, tests/test-toolkit-authoring.sh]
...
Bind native lineage and resolved execution recipes to atomic tasks. Preserve established interfaces and prove the requested behavior with the named regression check.
...
```bash
# eval_1: Bind native lineage and resolved execution recipes to atomic tasks
eval_1() {
  bash tests/test-toolkit-authoring.sh
}
...
success_criteria:
  - id: eval_1
    description: "Bind native lineage and resolved execution recipes to atomic tasks"
    runnable: bash
