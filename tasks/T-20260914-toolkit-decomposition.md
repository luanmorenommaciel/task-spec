...
agent: codex
parent: (none)
depends_on: [T-20260914-toolkit-contracts]
supersedes: (none)
touches_paths: [bin/taskspec]
creates_paths: [src/decompose, tests/test-toolkit-decompose.sh, tests/fixtures/toolkit]
...
Compile reviewed native decomposition and import legacy workspaces. Preserve established interfaces and prove the requested behavior with the named regression check.
...
```bash
# eval_1: Compile reviewed native decomposition and import legacy workspaces
eval_1() {
  bash tests/test-toolkit-decompose.sh
}
...
success_criteria:
  - id: eval_1
    description: "Compile reviewed native decomposition and import legacy workspaces"
    runnable: bash
