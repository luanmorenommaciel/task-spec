...
def managed_fixture(case, work, retained, args, env, ts, command):
    ...
    if case == 'run':
        # The positive execution fixture supplies an initialized host control
        # plane before entering a harness that protects Git metadata.
        ts('mesh', 'init')
        return
...
def prepare(args, retained):
    ...
    env['PATH'] = str(args.engine / 'bin') + os.pathsep + env.get('PATH', '')
    setup_rows = []
    ...
    # Stage the installed plugin's skill resources inside the permitted fixture
    # workspace. Native relative guide reads then need no external-directory grant.
    native_plugin = work / '.agents/skills/task-spec'
    (native_plugin / '.claude-plugin').mkdir()
    shutil.copyfile(args.engine / '.claude-plugin/plugin.json',
                    native_plugin / '.claude-plugin/plugin.json')
    instructions = (
    ...
    write(retained / 'fixture.json', {'workspace': str(work), 'case': args.case,
                                    ...
                                    'path_cli': shutil.which('taskspec', path=env['PATH']),
...
def main():
    ...
    if args.harness == 'claude':
        ...
        # Deny the pending tool and stop the turn after any recorded denial.
        # This changes no permission grants and never retries a denied action.
        settings = retained / 'guard-settings.json'
        guard = ROOT / 'tests/evals/toolkit/claude_permission_guard.py'
        settings.write_text(json.dumps({'hooks': {'PreToolUse': [{
            'matcher': '.*', 'hooks': [{'type': 'command', 'timeout': 10,
                'command': shlex.join([str(args.engine / '.taskspec/runtime/decompose/bin/python'),
                                      str(guard), '--events', str(retained / 'harness.stdout')])}]}]}}))
        # Restricted mode does not discover project skills. Load the installed
        # package explicitly for this session; keep its native permission mode.
        argv += ['--settings', str(settings), '--include-hook-events',
                 '--plugin-dir', str(work / '.agents/skills/task-spec')]
    ...
    prefix += [key + '=' + env[key] for key in ('TASKSPEC_HOME', 'TASKSPEC_DECOMPOSE_PYTHON', 'TASKSPEC_MESH_HELPER', 'TASKSPEC_MESH_ADAPTER_DIR', 'PATH') if key in env]
